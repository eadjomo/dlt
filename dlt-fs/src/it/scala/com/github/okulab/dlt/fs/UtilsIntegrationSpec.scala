...
class UtilsIntegrationSpec extends FeatureSpec with SparkSqlSpec with LocalClusterSpec with Matchers {
  ...
  feature("to test the uncompress")
  {
    ...
    scenario("test for unzip for tar with provided as one file")
    {
      ...
      mkdir("/uncompress")
      mkdir("/compress/compresstest.zip_TMP")
      mkdir("/compress/compresstest.zip")

      uploadResource("compresstest.zip","/compress/input")
      //uploadResource("test.zip","/compress/")
      utils.unCompressFiles(fsContext,"zip","hdfs://127.0.0.1:12345/tmp/hadoop/compress/input/compresstest.zip","hdfs://127.0.0.1:12345/tmp/hadoop/compress/compresstest.zip","hdfs://127.0.0.1:12345/tmp/hadoop//compress/compresstest.zip_TMP")
      val files = utils.getFilesFromPath( "hdfs://127.0.0.1:12345/tmp/hadoop/compress/compresstest.zip",true)
      files.foreach(p => println("outputfiles==>" + p.getPath))
      files.size should equal (3)
    }
